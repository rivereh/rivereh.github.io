...
RAIN_DROP.prototype = {
  ...
  render: function (context, toFront) {
    ...
    if (
      this.y >=
      this.height * (1 - (1 - this.scale) * this.THRESHOLD_RATE) - this.offset
    ) {
      context.lineWidth = 3
      context.globalAlpha =
...
HUMAN.prototype = {
  ...
  setParameters: function (toInit) {
    ...
    var position =
        this.renderer.getRandomValue(this.VERTICAL_OFFSET_RATE_RANGE) *
        this.height,
    ...
    this.vx =
      this.renderer.getRandomValue(this.VELOCITY_OFFSET) * this.direction
  ...
  renderSubstance: function (context) {
    ...
    if (
      ...
      (this.x > this.width + this.HORIZONTAL_OFFSET && this.direction > 0)
    ) {
...
RENDERER.drawSpaceNeedle = function () {
  const context = this.context

  context.save()
  context.fillStyle = 'rgba(150, 150, 150, 0.4)' // Light gray with some transparency
  ...
  // Body
  context.beginPath()
  context.moveTo(centerX - 10 * scale, baseY)
  context.lineTo(centerX - 5 * scale, baseY - 200 * scale)
  context.lineTo(centerX + 5 * scale, baseY - 200 * scale)
  context.lineTo(centerX + 10 * scale, baseY)
  context.closePath()
  context.fill()

  // Circular Top
  context.beginPath()
  context.arc(centerX, baseY - 220 * scale, 25 * scale, 0, Math.PI * 2)
  context.fill()
